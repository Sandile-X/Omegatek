...
| Issue | Solution |
|---|---|
| FFmpeg not installed | On Windows: `winget install Gyan.FFmpeg --accept-source-agreements`. Restart terminal after install. |
| Frames don't load | Check file paths, ensure local server is running (can't load from `file://`) |
| Animation is choppy / sluggish | **Do NOT use cover-fit full-screen canvas.** Cap canvas at frame native width: `canvasW = Math.min(window.innerWidth * 0.92, 1280)`. Remove DPR scaling entirely — it causes massive canvas dimensions on retina displays with no quality benefit for JPEG frames. |
| Canvas is blurry | DPR scaling is only needed if canvas > frame native resolution. Since frames are 1280x720, cap canvasW at 1280 and skip DPR. |
| `Cannot access 'currentFrame' before initialization` | Declare `let currentFrame = 0` BEFORE the `resizeCanvas()` function definition, not after. `resizeCanvas` calls `drawFrame(currentFrame)` so it must be declared first. |
| Scroll feels too fast/slow | Adjust `#subhero-section` height (200vh=fast, 500vh=slow, 800vh=cinematic) |
...
## Embedding Into an Existing Site (Sub-Hero Pattern)

When integrating the scroll animation into an **existing page** (not a standalone demo):

1. **Scope CSS with `#subhero-section` prefix** — avoids collisions with site's global `* { transition: all }` and other rules.
2. **No loader UI** — preload frames silently; call `drawFrame(0)` on first frame load.
3. **Frame path is relative to the root HTML file** — e.g. `demo/frames/frame_0001.jpg` if frames are in a `demo/` subfolder.
4. **No DPR scaling** — contained canvas (`Math.min(containerWidth, nativeFrameWidth)`) renders sharp at 1x and doesn't punish retina displays.
5. **drawFrame simplified** — with canvas AR matching frame AR, just `ctx.drawImage(img, 0, 0, canvasW, canvasH)`. No cover/contain math needed.
6. **CTA at end** — add `#subhero-cta` div inside the sticky container; show it when `progress >= 0.92`.
7. **Windows FFmpeg path** after winget install: `C:\Users\<user>\AppData\Local\Microsoft\WinGet\Packages\Gyan.FFmpeg_Microsoft.Winget.Source_8wekyb3d8bbwe\ffmpeg-8.1-full_build\bin\ffmpeg.exe`
