...
(function () {
   ...
   let LS_KEY      = 'ot_popup_dismissed';
   let WA_NUMBER   = '27736538207';
   let COOLDOWN_MS = 24 * 60 * 60 * 1000; // 24 hours
   let SHOW_DELAY  = 3000;                 // 3 seconds after load

   /* ── Paths (absolute so they work from any page depth) ──────────── */
   let MASCOT_IMG = '/images2/pops.png';
   let LOGO_IMG   = '/images2/OMEGATEK_SOLUTIONS_9-NO%20BG.png';
   ...
   function injectStyles() {
      ...
      style.textContent = [
         ...
         /* ▸ Card */
         '.ot-wp-card{position:relative;display:flex;flex-direction:row;max-width:980px;width:100%;border-radius:4px;overflow:hidden;box-shadow:0 24px 80px rgba(179,12,230,.25),0 0 0 1.5px rgba(179,12,230,.15);background:linear-gradient(135deg,#faf5ff 0%,#fff 60%,#f0fff4 100%);transform:translateY(40px) scale(.96);transition:transform .45s cubic-bezier(.22,1,.36,1);animation:ot-wp-shimmer 3s ease-in-out infinite alternate;}',
         ...
         /* ▸ Close button */
         '.ot-wp-close{position:absolute;top:14px;right:16px;z-index:10;width:38px;height:38px;border-radius:2px;border:1px solid rgba(179,12,230,.15);background:rgba(179,12,230,.06);color:#b30ce6;font-size:24px;line-height:36px;text-align:center;cursor:pointer;transition:all .25s ease;}',
         ...
         '.ot-wp-left{flex:0 0 320px;display:flex;flex-direction:column;align-items:center;justify-content:center;background:#f5efe6;padding:40px 26px;gap:18px;position:relative;overflow:hidden;}',
         ...
         '.ot-wp-mascot{width:250px;height:auto;border-radius:4px;filter:drop-shadow(0 8px 24px rgba(179,12,230,.18));transition:transform .4s cubic-bezier(.22,1,.36,1);animation:ot-wp-float 3s ease-in-out infinite;}',
         ...
         '.ot-wp-logo{width:150px;height:auto;opacity:.85;transition:opacity .3s;}',
         '.ot-wp-logo:hover{opacity:1;}',

         /* ▸ Right column */
         '.ot-wp-right{flex:1;padding:48px 44px 38px;display:flex;flex-direction:column;gap:18px;min-width:0;}',
         '.ot-wp-right h2{font-family:"Inter","Plus Jakarta Sans",system-ui,sans-serif;font-size:2.6rem;font-weight:800;letter-spacing:-.01em;color:#1a1a2e;margin:0;line-height:1.1;}',
         ...
         '.ot-wp-right p{font-family:"Montserrat",sans-serif;font-size:1.25rem;font-weight:500;color:#454552;margin:0;line-height:1.45;}',

         /* ▸ Textarea */
         '.ot-wp-textarea{width:100%;min-height:120px;max-height:180px;resize:vertical;border:1.5px solid rgba(179,12,230,.18);border-radius:3px;padding:16px 18px;font-family:"Montserrat",sans-serif;font-size:1.05rem;color:#333;background:#f8f5ff;outline:none;transition:border-color .25s,box-shadow .25s;}',
         ...
         '.ot-wp-nl-label{display:flex;align-items:center;gap:10px;font-size:.95rem;color:#555;cursor:pointer;font-family:"Montserrat",sans-serif;}',
         '.ot-wp-nl-label input[type=checkbox]{accent-color:#b30ce6;width:19px;height:19px;cursor:pointer;}',
         '.ot-wp-nl-email{width:100%;border:1.5px solid rgba(179,12,230,.18);border-radius:3px;padding:12px 16px;font-size:1rem;font-family:"Montserrat",sans-serif;background:#f8f5ff;color:#333;outline:none;transition:border-color .25s,box-shadow .25s,max-height .35s ease,opacity .35s ease,padding .35s ease;max-height:0;opacity:0;padding:0 16px;overflow:hidden;border-width:0;}',
         '.ot-wp-nl-email.ot-wp-show{max-height:56px;opacity:1;padding:12px 16px;border-width:1.5px;}',
         '.ot-wp-nl-email:focus{border-color:#b30ce6;box-shadow:0 0 0 3px rgba(179,12,230,.1);}',

         /* ▸ Send button */
         '.ot-wp-send{display:inline-flex;align-items:center;justify-content:center;gap:12px;width:100%;padding:18px 26px;border:none;border-radius:3px;font-family:"Inter","Plus Jakarta Sans",system-ui,sans-serif;font-size:1.15rem;font-weight:700;color:#fff;background:linear-gradient(135deg,#b30ce6 0%,#9333ea 50%,#25D366 100%);background-size:200% 200%;background-position:0% 50%;cursor:pointer;transition:all .35s ease;box-shadow:0 4px 16px rgba(179,12,230,.25);position:relative;overflow:hidden;}',
         ...
         '.ot-wp-send .fa-whatsapp{font-size:1.35rem;}',

         /* ▸ Success state */
         '.ot-wp-success{text-align:center;padding:48px 20px;display:flex;flex-direction:column;align-items:center;gap:18px;}',
         '.ot-wp-success-icon{font-size:3.6rem;color:#25D366;animation:ot-wp-pop .5s cubic-bezier(.22,1,.36,1);}',
         ...
         '.ot-wp-success h3{font-family:"Inter",system-ui,sans-serif;font-size:1.5rem;color:#1a1a2e;margin:0;}',
         '.ot-wp-success p{font-size:1rem;color:#666;margin:0;}',
         ...
         '  .ot-wp-card{flex-direction:column;max-width:460px;max-height:90vh;overflow-y:auto;}',
         '  .ot-wp-left{flex:0 0 auto;padding:26px 20px 18px;flex-direction:row;gap:16px;}',
         '  .ot-wp-mascot{width:120px;}',
         '  .ot-wp-logo{width:90px;}',
         '  .ot-wp-right{padding:24px 22px 26px;}',
         '  .ot-wp-right h2{font-size:1.85rem;}',
         '  .ot-wp-right p{font-size:1.05rem;}',
         '  .ot-wp-textarea{min-height:70px;font-size:1rem;}',
         '  .ot-wp-send{font-size:1.05rem;padding:16px 22px;}',
         '}'
      ].join('\n');
   ...
   function injectHTML() {
      ...
      div.innerHTML = [
         ...
         '    <h2>Got a <span>Tech Idea?</span></h2>',
         '    <p>Tell us what&rsquo;s up &mdash; we reply fast.</p>',
         '    <textarea class="ot-wp-textarea" id="ot-wp-message" placeholder="Your problem, idea, or question..." maxlength="1000"></textarea>',
   ...
   function spawnConfetti(container) {
      let colors = ['#b30ce6', '#9333ea', '#00f5d4', '#25D366', '#f59e0b', '#ef4444', '#3b82f6'];
   ...
   function show() {
      ...
      let ta = document.getElementById('ot-wp-message');
   ...
   function sendToWhatsApp(message) {
      ...
      let encoded = encodeURIComponent(lines.join('\n'));
      window.open('https://wa.me/' + WA_NUMBER + '?text=' + encoded, '_blank');
   }

   /* ── Show success state ───────────────────────────────────────────── */
   function showSuccess() {
      let right = document.getElementById('ot-wp-right-content');
      if (!right) return;
      right.innerHTML = [
      ...
      /* Confetti */
      let confettiContainer = right.querySelector('.ot-wp-confetti');
   ...
   function bindEvents() {
      ...
      /* Close button */
      let closeBtn = overlay.querySelector('.ot-wp-close');
      ...
      let nlCheck = document.getElementById('ot-wp-nl-check');
      let nlEmail = document.getElementById('ot-wp-nl-email');
      if (nlCheck && nlEmail) {
         nlCheck.addEventListener('change', function () {
            if (nlCheck.checked) {
               ...
               try {
                  let sbKeys = Object.keys(localStorage).filter(function (k) { return k.indexOf('supabase.auth') > -1; });
                  for (let i = 0; i < sbKeys.length; i++) {
                     let parsed = JSON.parse(localStorage.getItem(sbKeys[i]));
                     let email = parsed && parsed.user && parsed.user.email;
      ...
      /* Send button */
      let sendBtn = document.getElementById('ot-wp-send');
      let messageEl = document.getElementById('ot-wp-message');
      if (sendBtn && messageEl) {
         sendBtn.addEventListener('click', function () {
            let msg = messageEl.value.trim();
            if (!msg) {
